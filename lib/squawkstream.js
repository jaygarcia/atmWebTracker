...
function SquawkStream(sampleRate) {
  ...
  function patternAddress(pattern) { return (data[(pattern << 1) + 1] << 8) | data[(pattern << 1) + 2]; }
  ...
  function Channel(id) {
    ...
    var stackPattern = new Array(); // note 1
    var stackIndex   = 0;
    var repeatPoint = 0;
    ...
    // Force write pointer, used during pattern entry point setup
    ...
    function _play() {
      ...
      if (id === 3) { // just for a bit of throttling
        me.onTick(_getTickCount());
      }
      ...
      if(delay != 0) { 
        ...
        do {
          ...
          if(cmd < 64) {
            ...
            stackPattern[stackIndex] = pattern;
            counter = cmd == 252 ? 0 : readByte();
            pattern = readByte();
            stackPointer[stackIndex] = ptr;
            stackIndex++;
            ptr = patternAddress(pattern);
          } else if(cmd == 254) {
            // 254 : RETURN
            if(counter > 0) {
              // Repeat pattern
              ...
              if(stackIndex == 0) {
                ...
                pattern = stackPattern[stackIndex]; // note 1
              }
            }
          } else if(cmd == 255) {
            // 255 : EMBEDDED DATA
      ...
      if ((channelActiveMute & 0xF0) == 0)
      {
        ...
        if (repeatSong != 0)
        {
          ...
          //channel[0].ptr = patternAddress((channel[0].repeatPoint)) - (entryBase + 4);
          //channel[1].ptr = patternAddress((channel[1].repeatPoint)) - (entryBase + 4);
          //channel[2].ptr = patternAddress((channel[2].repeatPoint)) - (entryBase + 4);
          //channel[3].ptr = patternAddress((channel[3].repeatPoint)) - (entryBase + 4);
  ...
  function _setSource(source) {
    if(typeof source == "function") {
      ...
      // Auto-fill algorithm (for pattern addresses)
      var temp = new Array();
      ...
      // Set up entry patterns for each channel
      entryBase = (readByte(0) << 1) + 1;
      channel.forEach(function(e) { 
        e.jumpTo(patternAddress(readByte(entryBase + e.id()))); 
      });
    }
  }
